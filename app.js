(function () {
  ...
  function discoverAssets() {
    ...
    // 1. Try manifest.json (generated by scan-assets.sh, works on any server)
    return fetch("_assets/manifest.json")
      .then(function (r) {
        if (!r.ok) throw new Error("no manifest");
        return r.json();
      })
      .then(function (list) {
        var assets = list.filter(isMediaFile);
        if (assets.length === 0) throw new Error("empty manifest");
        return sortAssets(assets);
      })
      .catch(function () {
        // 2. Fall back to directory listing (works with Python's http.server)
        return fetch("_assets/")
          .then(function (r) {
            if (!r.ok) throw new Error("listing unavailable");
            return r.text();
          })
          .then(function (html) {
            ...
            var assets = Array.from(doc.querySelectorAll("a[href]"))
              .map(function (a) { return a.getAttribute("href").split("/").pop().split("?")[0]; })
              .filter(isMediaFile);
  ...
  function init() {
    ...
    discoverAssets().then(function (assets) {
      ...
      document.getElementById("gallery").innerHTML =
        '<p style="color:#888;padding:2rem;font-size:0.85rem">Could not load assets. Run <code>./scan-assets.sh ' + CLIENT_NAME.toLowerCase() + '</code> from the project root to generate a manifest, then reload.</p>';
    });
  }
